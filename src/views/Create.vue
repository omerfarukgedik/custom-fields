...
<script>
  ...
  export default {
    ...
    methods: {
      async submit() {
        ...
        if (isFormValid) {
          ...
          let photo = await axios.get("https://picsum.photos/240/100");
          ...
          const request = {
            id:
              ...
              Math.random()
                .toString(36)
                .substr(2, 9),
            photo: photo.request.responseURL,
          };
          ...
          await axios.post(`http://localhost:3000/blogs`, request);
          this.$router.push("/");
    ...
    async created() {
      try {
        this.loading = true;
        let res = await axios.get(`http://localhost:3000/custom_fields`);
